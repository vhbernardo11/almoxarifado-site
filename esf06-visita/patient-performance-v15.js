...
const visibleAgenda=x=>!String(x?.reason||'').startsWith('Roteiro inteligente:')||String(x?.notes||'').includes('acionamento manual');
...
window.fetch=async function(input,init){const s=urlString(input);if(s.includes('/acs-visita-api')){try{const u=new URL(s),api=u.searchParams.get('api');if(api==='patients')return response({rows:filterRows(await loadAll(),u)});if(api==='person'){const id=u.searchParams.get('id')||'';if(id){window.__ACS_ACTIVE_PERSON_ID=id;try{sessionStorage.setItem('acs_active_person_id',id)}catch{}}const j=await fastGet('detail',{id});return response(j)}if(api==='route'||api==='calendar'){const r=await previousFetch(input,init);if(!r.ok)return r;const j=await r.json();if(api==='route')j.rows=(j.rows||[]).filter(visibleAgenda);else j.agenda=(j.agenda||[]).filter(visibleAgenda);return response(j,r.status)}}catch(e){console.warn('[ACS Fast] usando API principal como contingência:',e)}}return previousFetch(input,init)};
